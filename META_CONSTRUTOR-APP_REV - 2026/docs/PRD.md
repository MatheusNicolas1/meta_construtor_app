...
* Webhook atualiza subscriptions e o app lê apenas do banco (não do frontend)
  ...
  EVIDÊNCIA: Edge function stripe-webhook/index.ts handlers: 1) checkout.session.completed (L73-115): extrai metadata (org_id, plan_id, user_id) → UPSERT subscriptions com onConflict stripe_subscription_id, 2) customer.subscription.updated (L117-151): UPDATE subscriptions (status, periods, trial_end, canceled_at), 3) customer.subscription.deleted (L153-181): UPDATE status='canceled', 4) invoice.payment_failed (L184-196): UPDATE status='past_due'. Metadata requerido: org_id/plan_id/user_id (M4.3). Legacy: profiles.subscription_status mantido (L104-113, L145-147, L172-178). Source of truth: subscriptions table. [M4 STEP 2] Price mapping: extrai price_id de subscription.items.data[0].price.id → busca plans WHERE stripe_price_id_monthly=? OR stripe_price_id_yearly=? → usa plan.id mapeado (não confia apenas em metadata.plan_id). Billing_cycle derivado de price_id match (monthly vs yearly).
