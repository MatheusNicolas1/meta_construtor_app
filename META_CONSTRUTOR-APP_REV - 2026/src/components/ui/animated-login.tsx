...
export function AnimatedLogin() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      // Navegação será feita pelo AuthContext após signIn
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleGoogleSignIn = async () => {
    setIsLoading(true);
    try {
      // Mock Google sign in - substituir por implementação real do Google OAuth
      await signIn("google.user@gmail.com", "google_auth_token");
      // Navegação será feita pelo AuthContext após signIn
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="min-h-screen w-screen bg-background relative overflow-hidden flex items-center justify-center">
      <SEO 
        ...
        description="Acesse sua conta com segurança no Meta Construtor" 
        canonical={window.location.href} 
      />
      
      {/* Background gradient effect - blue theme matching reference */}
      ...
      >
        ...
        >
          <div className="relative group">
            ...
            {/* Glass card background with blue-black gradient */}
            <div className="relative bg-gradient-to-b from-construction-blue/30 via-construction-blue/15 to-black/60 backdrop-blur-xl rounded-2xl p-6 border border-construction-blue/20 shadow-2xl overflow-hidden">
              ...
              <div className="text-center space-y-1 mb-6">
                ...
                >
                  Faça login para continuar no Meta Construtor
                </motion.p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex items-center justify-between pt-1">
                  <div className="flex items-center space-x-3">
                    ...
                    <label htmlFor="remember-me" className="text-xs text-white/60 hover:text-white/80 transition-colors duration-200 cursor-pointer select-none">
                      Lembrar de mim
                    </label>
                  </div>
                  
                  <div className="text-xs relative group/link">
                    <Link to="/recuperar-senha" className="text-white/60 hover:text-white transition-colors duration-200">
                      Esqueceu a senha?
                    </Link>
                  </div>
                </div>
                ...
                <motion.div
                  whileTap={{ scale: 0.98 }}
                  className="w-full group/button relative bg-white/10 p-px rounded-2xl backdrop-blur-lg overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 mt-5"
                  whileHover={{ y: -2 }}
                ...
                >
                  <button
                    type="submit"
                    disabled={isLoading}
                    className="relative w-full rounded-[1.15rem] px-8 py-3 text-lg font-semibold 
                      bg-white hover:bg-construction-orange text-black hover:text-white border-none 
                      transition-all duration-300 group-hover/button:shadow-lg overflow-hidden h-12 flex items-center justify-center"
                  >
                    {/* Button glow effect */}
                    <div className="absolute inset-0 bg-white/10 rounded-[1.15rem] blur-lg opacity-0 group-hover/button:opacity-70 transition-opacity duration-300" />
                    ...
                    <AnimatePresence mode="wait">
                      {isLoading ? (
                        ...
                        <motion.span
                          ...
                          animate={{ opacity: 1 }}
                          exit={{ opacity: 0 }}
                          className="opacity-90 group-hover/button:opacity-100 transition-opacity flex items-center justify-center"
                        >
                          Entrar
                          <span className="ml-3 opacity-70 group-hover/button:opacity-100 group-hover/button:translate-x-1.5 transition-all duration-300">
                            →
                          </span>
                        </motion.span>
                      )}
