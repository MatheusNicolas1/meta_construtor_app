...
const Atividades = () => {
  ...
  const { activitiesList, deleteActivity, isLoading } = useActivitiesSupabase();

  // Map activities for display
  const allActivities = activitiesList.map(activity => ({
    id: activity.id,
    nome: activity.titulo,
    categoria: activity.categoria || "Atividade",
    unidadeMedida: activity.unidade_medida || "Un",
    duracaoEstimada: "1 dia",
